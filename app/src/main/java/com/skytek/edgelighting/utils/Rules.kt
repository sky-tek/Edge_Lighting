...
class Rules(private val c: Context, private val prefs: SharedPreferences) {
    ...
    companion object {
        ...
        fun matchesChargingState(
            ...
            val ruleChargingState = prefs.getString(
                T.RULES_CHARGING_STATE, T.RULES_CHARGING_STATE_DEFAULT
            )
            if (ruleChargingState == T.RULES_CHARGING_STATE_DEFAULT) return true
            ...
            return (ruleChargingState == T.RULES_CHARGING_STATE_CHARGING && charging)
                    || (ruleChargingState == T.RULES_CHARGING_STATE_DISCHARGING && !charging)
        }
    }
    ...
    init {
        ...
        val startCalendar = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
    ...
    fun matchesBatteryPercentage(): Boolean {
        return (getBatteryStatus(c)?.getIntExtra(
            ...
            0
        ) ?: 100) > prefs.getInt(T.RULES_BATTERY, T.RULES_BATTERY_DEFAULT)
    }
